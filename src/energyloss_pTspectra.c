...
//  canvas->SetCanvasSize(1000, 1000);
  ...
  for (int iCent = ntest; iCent < ntest+1; ++iCent) {
    ...
    for (int ipt = 1; ipt <= n_dpt; ++ipt) {
      ...
      double ks_r = KS_statistic(placeholder_hist, data_pt[iCent], 0, minpt_comparison_threshold,
      ...
      double ks_n = KS_statistic(pp_reference, data_pt[iCent], dpt*ipt, minpt_comparison_threshold,
...
//    gPad->SetLogx();
    ...
    datCompPlot[0]->Scale(getIntegralAboveThreshold(opt_pt_hist[0], minpt_comparison_threshold)/
                          getIntegralAboveThreshold(datCompPlot[0], minpt_comparison_threshold));
    ...
    datCompPlot[1]->Scale(getIntegralAboveThreshold(opt_pt_hist[1], minpt_comparison_threshold)/
                          getIntegralAboveThreshold(datCompPlot[1], minpt_comparison_threshold));
    datCompPlot[1]->Draw("hist same");
...
//    bothCDF_rebin->Draw("al");
    ...
    datCompPlot[2]->Scale(getIntegralAboveThreshold(opt_pt_hist[2], minpt_comparison_threshold)/
                          getIntegralAboveThreshold(datCompPlot[2], minpt_comparison_threshold));
    datCompPlot[2]->Draw("hist same");

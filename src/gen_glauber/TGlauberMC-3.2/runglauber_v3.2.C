// Note -- search "ogrid" to modify thickness settings
                       ...
                       const char *fname)
{
  ...
  const bool fCalcAreaLocal = false;
  TGlauberMC *mcg = new TGlauberMC(sysA,sysB,signn);
  mcg->SetMinDistance(mind);
  mcg->SetBmin(bmin);
  mcg->SetBmax(bmax);
  
  TFile *out = TFile::Open(fname,"update",fname,9);
  if (!out) return;

  // create new TTree with MC Glauber information for input to IP-Jazma
  const Int_t lemonmaxNucleons = 250;
  Int_t       lemonnpart;
  Int_t       lemonncoll;
  Int_t       lemonnparta;
  Int_t       lemonnpartb;  
  ...
  bool        woundedproj[lemonmaxNucleons];
  ...
  bool        woundedtarg[lemonmaxNucleons];
  
  int n_fromfile = 0;
  TTree *lemon;
  if (!out->Get("lemon")) { // No existing tree
    lemon = new TTree("lemon","lemon");
    ...
    lemon->Branch("partpsi",lemonpartplane,"partpsi[10]/F");
    lemon->Branch("nproj",&lemonnproj,"nproj/I");
    lemon->Branch("ntarg",&lemonntarg,"ntarg/I");
    lemon->Branch("xproj",lemonxproj,"xproj[250]/F");
    lemon->Branch("yproj",lemonyproj,"yproj[250]/F");
    lemon->Branch("wproj",woundedproj,"wproj[250]/O");
    lemon->Branch("xtarg",lemonxtarg,"xtarg[250]/F");
    lemon->Branch("ytarg",lemonytarg,"ytarg[250]/F");
    lemon->Branch("wtarg",woundedtarg,"wtarg[250]/O");
    lemon->SetDirectory(out);
  } else { // Update existing tree
    lemon = static_cast<TTree*> (out->Get("lemon")->Clone());
    lemon->GetBranch("npart")->SetAddress(&lemonnpart);
    lemon->GetBranch("nparta")->SetAddress(&lemonnparta);
    lemon->GetBranch("npartb")->SetAddress(&lemonnpartb);
    lemon->GetBranch("ncoll")->SetAddress(&lemonncoll);
    lemon->GetBranch("b")->SetAddress(&lemonb);
    lemon->GetBranch("eccgaus")->SetAddress(lemoneccgaus);
    lemon->GetBranch("partpsi")->SetAddress(lemonpartplane);
    lemon->GetBranch("nproj")->SetAddress(&lemonnproj);
    lemon->GetBranch("ntarg")->SetAddress(&lemonntarg);
    lemon->GetBranch("xproj")->SetAddress(lemonxproj);
    lemon->GetBranch("yproj")->SetAddress(lemonyproj);
    lemon->GetBranch("wproj")->SetAddress(woundedproj);
    lemon->GetBranch("xtarg")->SetAddress(lemonxtarg);
    lemon->GetBranch("ytarg")->SetAddress(lemonytarg);
    lemon->GetBranch("wtarg")->SetAddress(woundedtarg);
    gRandom = static_cast<TRandom*> (out->Get("randomizer_endstate")->Clone());
    n_fromfile = lemon->GetBranch("b")->GetEntries();
    cout << "Beginning generation from event " << n_fromfile << endl;
  }
  ...
  // decrease ogrid resolution to greatly improve computation speed
  const int nPrintEvent = 10;
  if (ogrid) std::cout << Form("ogrid resolution :: %i", ogrid_resolution) << std::endl;
  for (Int_t ievent=n_fromfile; ievent<n; ++ievent) {
    if (ogrid && ievent % 10 == 0) std::cout << Form("event %i generated", ievent) << std::endl;
    ...
    lemonnproj   = nucA->GetN();
    lemonntarg   = nucB->GetN();
    ...
    lemonpartplane[0] = 0.0;
    lemonpartplane[1] = psi[1];
    lemonpartplane[2] = psi[2];
    lemonpartplane[3] = psi[3];
    lemonpartplane[4] = psi[4];
    lemonpartplane[5] = psi[5];
    lemonpartplane[6] = psi[6];
    lemonpartplane[7] = psi[7];
    lemonpartplane[8] = psi[8];
    lemonpartplane[9] = psi[9];
    ...
    for (Int_t i = 0; i<AN; ++i) {
      ...
      woundedproj[i] = nucleonA->IsWounded();
    }
    for (Int_t i = 0; i<BN; ++i) {
      TGlauNucleon *nucleonB=(TGlauNucleon*)(nucleonsB->At(i));
      lemonxtarg[i] = nucleonB->GetX();
      lemonytarg[i] = nucleonB->GetY();
      woundedtarg[i] = nucleonB->IsWounded();
    }
...
//    auto starttime = std::chrono::high_resolution_clock::now();
    if (ogrid) {
      ...
      const Int_t nbinsx = ogrid_resolution;
      const Int_t nbinsy = ogrid_resolution;
      const Double_t max_x = 7.5;
      double weightA[lemonmaxNucleons];
      double weightB[lemonmaxNucleons];
      ...
      for (int i=0;i<BN;i++) {
        weightB[i] = 1.0;
            ...
            /*
      for (int i=0;i<AN;i++) {
        weightA[i] = fgamma->GetRandom();
      }
      for (int i=0;i<BN;i++) {
        weightB[i] = fgamma->GetRandom();
      }
           */
        }
      ...
      TH2D* initA_hist = new TH2D(Form("initA_event%i",ievent), ";x;y;E [a.u.]", nbinsx, -max_x, max_x, nbinsy, -max_x, max_x);
      TH2D* initB_hist = new TH2D(Form("initB_event%i",ievent), ";x;y;E [a.u.]", nbinsx, -max_x, max_x, nbinsy, -max_x, max_x);
      for (Int_t ybin=1; ybin<=nbinsy; ybin++) {
        for (Int_t xbin=1; xbin<=nbinsx; xbin++) {
          ...
          long double contentA = 0.;
          long double contentB = 0.;
          long double holdContent = 0.;

          for (Int_t i = 0; i<AN; ++i) {
            TGlauNucleon *nucleonA=(TGlauNucleon*)(nucleonsA->At(i));
            holdContent = weightA[i] * smearing_function->Eval(nucleonA->GetX() - xval, nucleonA->GetY() - yval);
            
            contentA += holdContent;
            if (!nucleonA->IsWounded()) continue;   // skip non-wounded nucleons
            content += holdContent;
          }
          for (Int_t i = 0; i<BN; ++i) {
            TGlauNucleon *nucleonB=(TGlauNucleon*)(nucleonsB->At(i));
            holdContent = weightB[i] * smearing_function->Eval(nucleonB->GetX() - xval, nucleonB->GetY() - yval);
            
            contentB += holdContent;
            if (!nucleonB->IsWounded()) continue;   // skip non-wounded nucleons
            content += holdContent;
          }
          inited_hist->SetBinContent(xbin, ybin, content);
          initA_hist->SetBinContent(xbin, ybin, contentA);
          initB_hist->SetBinContent(xbin, ybin, contentB);
        }
      }
      inited_hist->Write();
      if (inited_hist) delete inited_hist;
      initA_hist->Write();
      if (initA_hist) delete initA_hist;
      initB_hist->Write();
      if (initB_hist) delete initB_hist;
    } // End grid calculation
    // Print time taken in the current loop
//    auto looptime_clock = std::chrono::high_resolution_clock::now();
//    int looptime = std::chrono::duration_cast<std::chrono::milliseconds>(looptime_clock - starttime).count();
//    std::cout << Form("Loop in Glauber: %02d:%02d.%03d", looptime/60000, looptime/1000 - 60*(looptime/60000), looptime%1000) << std::endl;
    
    // Also, calculate the area via the glauber built-in methods
    if (fCalcAreaLocal) {
      const Int_t nbins=1000;
      const Double_t ell=10;
      const Double_t da=2*ell*2*ell/nbins/nbins;
      const Double_t d2 = mcg->GetXSectEvent()/(TMath::Pi()*10); // in fm^2
      const Double_t r2 = d2/4.;
      const Double_t mx = mcg->GetMeanX();
      const Double_t my = mcg->GetMeanY();
      ...
      for (Int_t i = 0; i<AN; ++i) {
        ...
        if (!nucleonA->IsWounded())
          continue;
        if (nucleonA->GetNColl()==0) {// == kNc, which is switched off for us
          std::cout << "----------------------------Triggered on NColl = 0!" << std::endl;
          continue;
      ...
      for (Int_t i = 0; i<BN; ++i) {
        ...
        if (!nucleonB->IsWounded())
          continue;
        if (nucleonB->GetNColl()==0) {// == kNc, which is switched off for us
          std::cout << "----------------------------Triggered on NColl = 0!" << std::endl;
          continue;
      ...
      std::cout << "AreaO: " << overlap2*da << std::endl;
      std::cout << "AreaA: " << overlap1*da << std::endl;
    }
    
//    int areatime = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - looptime_clock).count();
//    std::cout << Form("Loop in Area: %02d:%02d.%03d", areatime/60000, areatime/1000 - 60*(areatime/60000), areatime%1000) << std::endl;
    
  } // end loop over events
  
  // Additionally, store the current state of gRandom
  TRandom *randomizer = static_cast<TRandom*>(gRandom->Clone());
  randomizer->SetName("randomizer_endstate");
  randomizer->Write("", TObject::kOverwrite);
  lemon->Write("", TObject::kOverwrite);
  out->Close();
  delete out;
}
 ...
 cmscheck: /* start over here in case shift was too large */
  ...
  if (fN==1) { //special treatment for proton
    ...
    if (fNucCounter == -1) {
      ...
      if (tmpname=="He3") {
        snprintf(filename,20,"he3_plaintext.dat");
      } else if (tmpname=="H3") {
        snprintf(filename,20,"h3_plaintext.dat");
      } else if (tmpname=="He4") {
        snprintf(filename,20,"he4_plaintext.dat");
      } else if (tmpname=="C") {
        snprintf(filename,20,"carbon_plaintext.dat");
      } else if (tmpname=="O") {
        snprintf(filename,20,"oxygen_plaintext.dat");
      }

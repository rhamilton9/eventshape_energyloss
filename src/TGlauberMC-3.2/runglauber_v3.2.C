                       ...
                       const char *fname)
{
  ...
  if (!out->Get("lemon")) { // No existing tree
    ...
    lemon->GetBranch("eccpoint")->SetAddress(lemoneccpoint);
    ...
    lemon->GetBranch("zproj")->SetAddress(lemonyproj);
    lemon->GetBranch("xtarg")->SetAddress(lemonxtarg);
    lemon->GetBranch("ytarg")->SetAddress(lemonytarg);
    lemon->GetBranch("ztarg")->SetAddress(lemonytarg);
  ...
  for (Int_t ievent=n_fromfile; ievent<n; ++ievent) {
	...
	for (Int_t xbin=1; xbin<=nbinsx; xbin++) {
	  ...
	  long double content = 0.;  // sum the contributions from all wounded nucleons
        ...
        for (Int_t i = 0; i<AN; ++i) {
          TGlauNucleon *nucleonA=(TGlauNucleon*)(nucleonsA->At(i));
              contentA += weightA[i] * smearing_function->Eval(nucleonA->GetX() - xval, nucleonA->GetY() - yval);
          if (!nucleonA->IsWounded()) continue;   // skip non-wounded nucleons
          content += weightA[i] * smearing_function->Eval(nucleonA->GetX() - xval, nucleonA->GetY() - yval);
        }
        for (Int_t i = 0; i<BN; ++i) {
          TGlauNucleon *nucleonB=(TGlauNucleon*)(nucleonsB->At(i));
          contentB += weightB[i] * smearing_function->Eval(nucleonB->GetX() - xval, nucleonB->GetY() - yval);
          if (!nucleonB->IsWounded()) continue;   // skip non-wounded nucleons
          content += weightB[i] * smearing_function->Eval(nucleonB->GetX() - xval, nucleonB->GetY() - yval);
        }
